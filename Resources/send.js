...
(function () {
  ...
  async function findAndOpenVerifiedChat(username, label, isGroup) {
    ...
    for (const cand of candidates) {
      ...
      // Даём времени на загрузку сообщений в треде — список рендерится
      // асинхронно после открытия чата, и разделитель «Сегодня» может
      // ещё не появиться к моменту проверки.
      await sleep(2500);
